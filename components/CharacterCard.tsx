...
export const CharacterCard: React.FC<CharacterCardProps> = ({ data }) => {
  return (
    <a
      ...
      className="flex bg-gray-800 rounded-lg shadow-lg overflow-hidden border border-gray-700 transition-all transform hover:scale-105 hover:border-teal-500 duration-300 text-white no-underline"
      aria-label={`View details for ${data.name}`}
    >
      <div className="flex-shrink-0 w-32">
        {data.image?.url ? (
          <img src={data.image.url} alt={data.name} className="w-full h-full object-cover object-top" />
        ) : (
      ...
      <div className="p-4 flex-1">
        <h3 className="font-bold text-lg text-teal-400">{data.name}</h3>
